...
import org.foobar2000.api.GUID_from_text;
import org.foobar2000.api.file_info;
import org.foobar2000.api.file_info_impl;
import org.foobar2000.api.metadb_handle;
import org.foobar2000.plugin.FBAPI;
import org.foobar2000.plugin.UIESWTPanel;


public class UIEBrowserPanel extends UIESWTPanel {
		...
		static public final GUID GUID = new GUID_from_text("B09EB4FD-647F-4459-8940-3118B4B4BCB1");
		...
		public void on_playback_new_track(final metadb_handle item) {
			...
			display.asyncExec(new Runnable() {
				...
				public void run() {
					...
					String release_id = finfo.meta_get("DISCOGS_RELEASE_ID", 0);
					if(release_id != null) {
						String url = "http://www.discogs.com/release/" + release_id;
		...
		void tagItems(final Vector<metadb_handle> items) {
			// stupid test to see if some foobar action that must take place in the 
			// main fb thread is exeuted correctly when code is triggeret from SWT UI Thread
			display.asyncExec(new Runnable() {
